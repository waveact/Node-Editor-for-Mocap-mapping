...
class Knob(QGraphicsItem):
    ...
    def paint(self, painter, option, widget):
        ...
        self.bbox = self.boundingRect()
        self.bbox2 = self.boundingRect()
        painter.lineColor = QtGui.QColor(255, 0, 0)
        ...
        painter.setPen(QtGui.QPen(painter.lineColor))
        painter.setBrush(QtGui.QBrush(self.fillColor))
        painter.drawRect(self.bbox)
        #painter.drawRect(10,10,100,50)
        #painter.drawRoundRect(50, 50, 50, 50, 100, 100)
        ...
        if self.flow == FLOW_LEFT_TO_RIGHT:
            x = self.bbox.right() + self.margin
        elif self.flow == FLOW_RIGHT_TO_LEFT:
            x = self.bbox.left() - self.margin - textSize.width()
        else:
    ...
    def mousePressEvent(self, event):
        ...
        #print self.parentItem.scene.itemAt(QCursor.pos())
        #print self.parentItem().scene().itemAt(event.scenePos(), QTransform()).name
        #self.button = self.parentItem().scene().addWidget(PySide2.QtWidgets.QPushButton( "Start Record" ))
        #print self.bbox
    ...
    def mouseMoveEvent(self, event):
        ...
        #self.button.move(event.scenePos())
        #self.button.setGeometry(20, 0, 200, 100);
    ...
    def mouseReleaseEvent(self, event):
        ...
        if event.button() == QtCore.Qt.MouseButton.LeftButton:
            ...
            target = scene.itemAt(event.scenePos(), QTransform())
...
class OutputKnob(Knob):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #print kwargs
        #self.value = kwargs.get("value", "aa" )
        #print self.value
