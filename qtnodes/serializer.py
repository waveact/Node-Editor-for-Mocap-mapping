...
def serializeSlider(slider):
    """Return the Node as native Python datatypes, fit for json."""
    return {
        "class": slider.__class__.__name__,
        "node": slider.node.uuid,
        "name": slider.name,
...
def serializeScene(scene):
    ...
    sliders = [i for i in scene.items() if isinstance(i, Slider)]
    data = {
        "nodes": [serializeNode(node) for node in nodes],
        "edges": [serializeEdge(edge) for edge in edges],
        "sliders": [serializeSlider(slider) for slider in sliders],
    }
    return data
...
def reconstructScene(graphWidget, sceneData):
    ...
    for sliderData in sceneData["sliders"]:
        targetNode = graphWidget.getNodeById(sliderData["node"])
        targetSlider = targetNode.slider(sliderData["name"])
        targetSlider.sliderValue = sliderData["value"]
        targetSlider.text = str(sliderData["value"])
        #print targetNode.slider(sliderData["name"])
        #targetNode.slider(sliderData["name"]).sliderValue = sliderData["value"]
        #targetNode.slider(sliderData["name"]).text = str(sliderData["value"])
        #targetNode.slider.update()
